...
export function ArticleCard({ source, sourceName, title, url, score, comments, publishedAt, isTrending, coverImage }: ArticleProps) {
  const getSourceDisplay = () => {
    switch (source) {
      case 'reddit': return { label: sourceName || 'r/programming', color: 'bg-primary-container text-on-primary-container' };
      case 'hn': return { label: 'Hacker News', color: 'bg-tertiary-container text-on-tertiary-container' };
      case 'devto': return { label: 'daily.dev', color: 'bg-[#e1e2e9] text-[#505257]' };
      case 'hashnode': return { label: 'Hashnode', color: 'bg-secondary-container text-on-secondary-container' };
      default: return { label: source, color: 'bg-stone-100 text-stone-500' };
    }
  };
    ...
    <article className="group bg-transparent p-0 transition-all duration-300 flex items-start lg:items-center gap-6 lg:gap-14 border-none">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-4 mb-4 flex-wrap">
          <span className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.2em] rounded-none ${color}`}>
            {label}
          </span>
